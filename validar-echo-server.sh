...
NETWORK="$(
  docker inspect -f '{{range $k, $v := .NetworkSettings.Networks}}{{println $k}}{{end}}' "$SERVER_CONTAINER" \
    | head -n 1 || true
)"
...
MSG="tp0-echo-test-$(date +%s%N)"

REPLY="$(
  docker run --rm --network "$NETWORK" alpine:3.19 sh -lc "
    set -e
